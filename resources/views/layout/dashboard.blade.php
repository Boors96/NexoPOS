...
<body <?php echo in_array(app()->getLocale(), config('nexopos.rtl-languages')) ? 'dir="rtl"' : ''; ?>>
    <div class="h-full w-full flex flex-col">
        <div id="dashboard-body" class="overflow-hidden flex flex-auto">
            <div id="dashboard-aside" v-cloak v-if="sidebar === 'visible'"
                ...
                <div class="ns-scrollbar overflow-y-auto h-full text-sm">
                    ...
                    <ul>
                        @foreach ($menus->getMenus() as $identifier => $menu)
                            @if (
                                (isset($menu['permissions']) && Auth::user()->allowedTo($menu['permissions'], 'some')) ||
                                        ...
                                        @foreach ($menu['childrens'] as $identifier => $menu)
                                            @if (
                                                ...
                                                <ns-submenu
                                                    ...
                                                    href="{{ $menu['href'] }}" id="submenu-{{ $identifier }}">
                                                    {{ $menu['label'] }}</ns-submenu>
                                            @endif
                                        @endforeach
                                    @endif
                                </ns-menu>
            ...
            <div id="dashboard-overlay" v-if="sidebar === 'visible'" @click="closeMenu()"
                class="z-40 w-full h-full md:hidden absolute" style="background: rgb(51 51 51 / 25%)"></div>
            <div class="flex flex-auto flex-col overflow-hidden" id="dashboard-body">
                ...
                <div class="p-2 text-xs flex justify-end text-gray-500">
                    {!! Hook::filter(
                        'ns-footer-signature',
                        sprintf(
                            __('You\'re using <a tager="_blank" href="%s" class="hover:text-blue-400 mx-1 inline-block">NexoPOS %s</a>'),
